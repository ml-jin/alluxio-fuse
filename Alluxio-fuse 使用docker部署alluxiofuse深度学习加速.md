...
[![Slack](https://img.shields.io/badge/slack-alluxio--community-blue.svg?logo=slack)](https://www.alluxio.io/slack) [![Docker Pulls](https://img.shields.io/docker/pulls/alluxio/alluxio.svg?logo=docker&color=166FDB)](https://hub.docker.com/r/alluxio/alluxio) [![GitHub edit source](https://img.shields.io/badge/github-edit%20source-blue.svg?logo=github&color=166FDB)](https://github.com/Alluxio/alluxio/blob/branch-2.4/docs/cn/deploy/Running-AlluxioFuse-On-Docker.md)

- 概述
  - [什么是FUSE和AlluxioFuse](https://docs.alluxio.io/os/user/stable/cn/deploy/Running-AlluxioFuse-On-Docker.html#什么是fuse和alluxiofuse)
- 在Docker上单机部署AlluxioFuse
  - [前提条件](https://docs.alluxio.io/os/user/stable/cn/deploy/Running-AlluxioFuse-On-Docker.html#前提条件)
  - 单机部署
    - [设置底层存储系统](https://docs.alluxio.io/os/user/stable/cn/deploy/Running-AlluxioFuse-On-Docker.html#设置底层存储系统)
    - [设置RAMFS](https://docs.alluxio.io/os/user/stable/cn/deploy/Running-AlluxioFuse-On-Docker.html#设置ramfs)
    - [启动Alluxio master](https://docs.alluxio.io/os/user/stable/cn/deploy/Running-AlluxioFuse-On-Docker.html#启动alluxio-master)
    - [启动Alluxio worker](https://docs.alluxio.io/os/user/stable/cn/deploy/Running-AlluxioFuse-On-Docker.html#启动alluxio-worker)
    - [设置AlluxioFuse挂载路径](https://docs.alluxio.io/os/user/stable/cn/deploy/Running-AlluxioFuse-On-Docker.html#设置alluxiofuse挂载路径)
    - [启动AlluxioFuse](https://docs.alluxio.io/os/user/stable/cn/deploy/Running-AlluxioFuse-On-Docker.html#启动alluxiofuse)
  - [测试Alluxio集群](https://docs.alluxio.io/os/user/stable/cn/deploy/Running-AlluxioFuse-On-Docker.html#测试alluxio集群)
- AlluxioFuse调优
  - [FUSE层优化配置项](https://docs.alluxio.io/os/user/stable/cn/deploy/Running-AlluxioFuse-On-Docker.html#fuse层优化配置项)
  - AlluxioFuse相关优化配置项
    - [jnifuse](https://docs.alluxio.io/os/user/stable/cn/deploy/Running-AlluxioFuse-On-Docker.html#jnifuse)
    - [client端缓存](https://docs.alluxio.io/os/user/stable/cn/deploy/Running-AlluxioFuse-On-Docker.html#client端缓存)
- 应用实践：在阿里云上基于kubernetes集群搭建四机八卡深度学习环境
  - 实验环境说明
    - [硬件环境](https://docs.alluxio.io/os/user/stable/cn/deploy/Running-AlluxioFuse-On-Docker.html#硬件环境)
    - [数据集](https://docs.alluxio.io/os/user/stable/cn/deploy/Running-AlluxioFuse-On-Docker.html#数据集)
    - [数据存储](https://docs.alluxio.io/os/user/stable/cn/deploy/Running-AlluxioFuse-On-Docker.html#数据存储)
  - 部署流程
    - [前提条件](https://docs.alluxio.io/os/user/stable/cn/deploy/Running-AlluxioFuse-On-Docker.html#前提条件-1)
    - [设置虚拟内存文件系统](https://docs.alluxio.io/os/user/stable/cn/deploy/Running-AlluxioFuse-On-Docker.html#设置虚拟内存文件系统)
    - [设置底层文件系统](https://docs.alluxio.io/os/user/stable/cn/deploy/Running-AlluxioFuse-On-Docker.html#设置底层文件系统)
    - [下载helm-chart](https://docs.alluxio.io/os/user/stable/cn/deploy/Running-AlluxioFuse-On-Docker.html#下载helm-chart)
    - [配置Alluxio集群（基础版）](https://docs.alluxio.io/os/user/stable/cn/deploy/Running-AlluxioFuse-On-Docker.html#配置alluxio集群基础版)
    - [启动Alluxio集群](https://docs.alluxio.io/os/user/stable/cn/deploy/Running-AlluxioFuse-On-Docker.html#启动alluxio集群)
    - [配置Alluxio集群（进阶版）](https://docs.alluxio.io/os/user/stable/cn/deploy/Running-AlluxioFuse-On-Docker.html#配置alluxio集群进阶版)
- [总结与展望](https://docs.alluxio.io/os/user/stable/cn/deploy/Running-AlluxioFuse-On-Docker.html#总结与展望)
- [参考文献](https://docs.alluxio.io/os/user/stable/cn/deploy/Running-AlluxioFuse-On-Docker.html#参考文献)
...
用户空间文件系统（**F**ilesystem in **Use**rspace， 简称**FUSE**）是一个面向类Unix系统的软件接口，它使得普通用户无需更改内核代码就能创建自己的文件系统[2]。 FUSE包括kernel提供的内核模块和用户态的[libfuse库](https://github.com/libfuse/libfuse)。libfuse库提供了一套标准的文件系统接口。

Alluxio基于这套文件系统接口实现了简单易用的**AlluxioFuse**。AlluxioFuse给予了Alluxio更多易用性和灵活性，使得Alluxio能够扩展到更多场景。 由于libfuse基于C，而Alluxio基于Java，我们可以采用JNI或者JNR等不同方式使Alluxio对接libfuse。 本实验版本是使用JNI来直接对接Alluxio和libfuse，而不是Alluxio发行版默认的JNR。
...
在这个示例中，`ramfs`的大小设置为了`1G`，也可以根据实验环境设置为其他数值。 此后在Worker的设置中，交给Worker管理的堆外内存大小需要与其一致。
